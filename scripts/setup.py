"""Idempotent first-run bootstrap for the persona-builder.
...
Used by `/persona-studio` and `/persona-setup` slash commands. Also safe to run
standalone:
...
def setup_plan(
    ...
    steps.append(Step(
        name="pip_install",
        description="Install persona-builder and dev extras",
        action=lambda: _run(
    ...
    steps.append(Step(
        ...
        action=lambda: _run(
            [str(py), "-m", "persona_builder.cli", "setup"],
            cwd=repo_root,
        ),
    ))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Bootstrap persona-builder.")
    parser.add_argument(
        "--repo-root",
        type=Path,
    ...
    print(f"persona-builder bootstrap — repo_root={repo_root}")
    ...
    if code == 0:
        print("\n[SUCCESS] persona-builder is ready. Run /persona-studio to start.")
    return code
