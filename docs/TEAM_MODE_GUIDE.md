...
**예시**: 주지훈 패널에 직접 "2009 사건 이후 변한 점이 있다면?" 라고 적으면, 해당 teammate가 persona 톤으로 답.

### 2. Main 창에 "<aside>" 코멘트

Main Claude가 현재 진행 중일 때, main 창에 `<aside> 주지훈 님 발언을 좀 더 감정적으로 다시 받고 싶어요` 라고 쓰면 퍼실리테이터가 다음 dispatch에 반영.

### 3. SendMessage 직접 (고급)

Claude에게 "`SendMessage(to='avatar-ju-ji-hoon', content='...')` 실행해주세요" 라고 요청하면 특정 teammate에게 메시지 주입.
