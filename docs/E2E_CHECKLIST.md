...
**기대 출력 마지막 줄**: `[SUCCESS] persona-studio is ready. Run /persona-studio to start.`
...
```bash
python -m persona_studio.cli extract sample_private
```
...
```bash
python -m persona_studio.cli extract "../etc/passwd"
```
...
- `python -m persona_studio.cli extract sample_private` 호출
