...
# 중요한 미팅 전에, 그 사람들과 미리 회의를 한다

Persona Studio는 **Claude Code 플러그인**이에요:

1. **AI 아바타 생성** — 인터뷰·팟캐스트·유튜브·기사 같은 자료(또는 이름만)로 실제 사람의 말투·관점을 학습한 아바타를 만듭니다.
2. **회의나 토론 실행** — 여러 아바타가 각자 별도 터미널 창(pane)에서 **동시에** 대화합니다.
3. **Word·PowerPoint 자동 생성** — 대화 전문을 담은 Word 문서와 요약 PowerPoint가 끝나고 바로 나옵니다.

**코딩 지식이 전혀 필요 없어요.** `/persona-studio:studio` 같은 슬래시 커맨드 하나 입력하고, 화살표 키로 메뉴를 고르면 나머지는 자동으로 됩니다.
...
<table>
<tr>
<td><strong>투자자 피칭 리허설</strong></td>
<td>투자자들의 공개 인터뷰로 아바타를 만들고, 내 덱을 피칭해보세요. 실제 미팅 전에 그들이 어떻게 반박할지 미리 들어볼 수 있어요.</td>
</tr>
<tr>
<td><strong>의사결정 스트레스 테스트</strong></td>
<td>CEO·CFO·개발총괄 아바타를 만들어 제안을 던져놓으세요. 그들끼리 반박·수용하게 놔두면, 살아남는 게 진짜 답입니다.</td>
</tr>
<tr>
<td><strong>면접 준비</strong></td>
<td>면접관의 링크드인·팟캐스트로 아바타를 만들어 역할극 면접을 돌려보세요. 끝나면 만족도 점수까지 나옵니다.</td>
</tr>
<tr>
<td><strong>전략 덱 초안 작성</strong></td>
<td>팀 회의를 한 번 돌리세요. 대화 전문 Word + 요약 PowerPoint가 자동으로 만들어져서, 그대로 초안으로 쓸 수 있어요.</td>
</tr>
<tr>
<td><strong>소설·시나리오 집필</strong></td>
<td>캐릭터 아바타들을 만들어 특정 장면을 토론시켜 보세요. 나온 transcript를 초안으로 쓸 수 있습니다.</td>
</tr>
</table>
...
- **비개발자인 창업자·컨설턴트·작가·연구자** — 중요한 대화를 준비하는 모든 사람.
- **PM·기획자** — 이해관계자 미팅 리허설.
- **제2외국어 학습자** — 실제 미팅 전에 원어민 아바타와 논쟁 연습.
- **Claude Code로 개발하는 사람** — Agent Teams + Ralph loop 패턴의 실사용 예제.
...
샘플 아바타가 이미 포함돼 있어서 자료 업로드 없이 바로 시도할 수 있습니다:

1. 실행: `/persona-studio:studio`
2. 선택: **"시뮬레이션 실행"** → **"토론"**
3. 주제: `원격근무의 미래`
4. 참가자 1: `sample_private` (포함된 샘플)
5. 참가자 2: 새 아바타를 먼저 만들거나, 빠른 테스트로 `sample_private`를 두 번 선택
6. 3라운드 토론 진행
7. 결과: `simulations/<주제>/<타임스탬프>.md` (+ 같은 이름의 `.docx`, `.pptx`)
