...
# /simulate-meeting-team

Split-panes version of `/simulate-meeting`. Each persona becomes a named
teammate running in its own pane. Sequential version stays at `/simulate-meeting`
...
2. **Inside a tmux session AND teammate split-panes mode active?**
   ...
   - If check fails: tell user "현재 세션은 in-process 모드입니다. 새 터미널에서 `claude --teammate-mode split-panes` 로 재시작 후 다시 `/persona-studio` → 이 메뉴로 진입하세요."
   - Offer AskUserQuestion: `세션 재시작 안내 자세히 보기` / `sequential 모드로 전환 (/simulate-meeting)` / `취소`.
...
1. `Read(.claude/agents/persona-<p>.md)` → full persona body.
...
## Step 5 — Save transcript (same markdown contract as /simulate-meeting)
...
Same pipeline as `/simulate-meeting` Step 5:
