...
- **Native subagent** (`subagent_type="persona-<slug>"`): works ONLY if
  ...
  populated at session-start from `.claude/agents/`; files added mid-session are
  NOT auto-registered.

- **Dynamic persona** (recommended default):
  `subagent_type="general-purpose"` + `Read(.claude/agents/persona-<slug>.md)`
...
For `r in 1..N`:
  For each participant `p` in declared order:
    ...
    **Dynamic persona invocation** (default):
    1. `Read(.claude/agents/persona-<p>.md)` — get the full persona body.
    2. Invoke:
       ```
       Agent(
    ...
    3. The composed prompt structure:
       ...
       [PERSONA FILE — inline]
       <full body of .claude/agents/persona-<p>.md, including runtime rules>
